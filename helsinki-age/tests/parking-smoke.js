...
assert.match(await page.locator('#building-parking-context').textContent(),/not whether this specific building has its own parking/);
assert.match(await page.locator('#building-parking-context').textContent(),/within 200 m/);
await page.locator('#building-parking-context').scrollIntoViewIfNeeded();await mkdir('artifacts/parking',{recursive:true});await page.screenshot({path:'artifacts/parking/desktop.png'});
...
await page.setViewportSize({width:390,height:844});await page.locator('#building-parking-context').scrollIntoViewIfNeeded();await page.screenshot({path:'artifacts/parking/mobile.png'});
assert.equal(await page.locator('#building-card').evaluate(e=>e.scrollWidth<=e.clientWidth),true);
await page.evaluate(()=>__atlas.showBuilding({address:'Outside coverage',position:[24.7,60.2]}));assert.match(await page.locator('#building-parking-context').textContent(),/No digitized data/);
