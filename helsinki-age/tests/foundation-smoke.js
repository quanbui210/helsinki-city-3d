...
try{
 ...
 await page.evaluate(()=>__atlas.nav.flyTo([24.95,60.17],550));await page.waitForFunction(()=>!__atlas.nav.flight&&__atlas.streetTrees.loaded.size>0&&__atlas.realityMesh&&__atlas.distantContext,{},{timeout:60000});await page.waitForFunction(()=>__atlas.viewer.scene.globe.tilesLoaded,null,{timeout:60000});
 ...
 for(const layer of ['age','noise','energy','use','price','parking']){
   await page.evaluate(layer=>{__atlas.slider.set(__atlas.slider.max);__atlas.switchLayer(layer);},layer);await page.waitForTimeout(350);
