...
try{
 ...
 await setup();await page.evaluate(()=>{for(let i=0;i<6;i++)selectTest(`Rapid ${i}`);searchTest.close();});await page.waitForTimeout(200);assert.equal(calls,0);
 await page.evaluate(()=>{selectTest('Fast 1');searchTest.search();});await page.waitForFunction(()=>testRoot.textContent.includes('View listing'));assert.equal(calls,1);
 ...
 await page.evaluate(()=>{selectTest('Slow 1');searchTest.search();});await page.waitForTimeout(200);await page.evaluate(()=>{selectTest('Next 2');searchTest.search();});await page.waitForFunction(()=>testRoot.querySelector('strong')?.textContent==='Next 2');await page.waitForTimeout(1000);assert.equal(await page.evaluate(()=>testRoot.querySelector('strong').textContent),'Next 2');
 ...
 await page.evaluate(()=>localStorage.clear());await setup();await page.evaluate(()=>{selectTest('Fast 1');searchTest.search();});await page.waitForFunction(()=>testRoot.textContent.includes('View listing'));assert.equal(calls,before+2);
 await page.evaluate(()=>{for(const key of Object.keys(localStorage)){if(key.startsWith('helsinki-lens:listings:')){const entry=JSON.parse(localStorage[key]);entry.timestamp=Date.now()-86400001;localStorage[key]=JSON.stringify(entry);}}});await setup();await page.evaluate(()=>{selectTest('Fast 1');searchTest.search();});await page.waitForFunction(()=>testRoot.textContent.includes('View listing'));assert.equal(calls,before+3);
 console.log('PASS: idle until search, synchronous cache, reload persistence, TTL, cleared cache, deduplicated promises and stale-response protection');
