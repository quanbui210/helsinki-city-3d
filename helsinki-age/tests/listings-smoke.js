...
try{
 ...
 assert.match(await page.locator('.listing-status').textContent(),/Checking/);
 await page.waitForFunction(()=>document.querySelector('.listing-status')?.textContent.startsWith('1 search'));
 ...
 mode='empty';await page.evaluate(r=>__atlas.showBuilding(r),records[1]);await page.waitForFunction(()=>document.querySelector('.listing-status')?.textContent.startsWith('No indexed'));
 assert.equal(await page.locator('.listing-manual-links a').count(),5);
 mode='failure';await page.evaluate(r=>__atlas.showBuilding(r),records[2]);await page.waitForFunction(()=>document.querySelector('.listing-status')?.textContent.includes('not connected'));
 ...
 await page.locator('#close-building').click();const before=requests;await page.evaluate(r=>{__atlas.showBuilding(r);document.querySelector('#close-building').click();},records[0]);await page.waitForTimeout(500);assert.equal(requests,before,'closing before debounce cancels lookup');
