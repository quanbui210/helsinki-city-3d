...
test('Generated spatial tiles match every manifest row',()=>{
 ...
 for(const tile of tiles.root.children){const rows=m.filter(r=>r.tileContentUri===tile.content.uri),b=fs.readFileSync(`${PUBLIC}tileset/${tile.content.uri}`);assert.equal(b.toString('utf8',0,4),'b3dm');assert.equal(b.readUInt32LE(4),1);assert.equal(b.readUInt32LE(8),b.length);const ftl=b.readUInt32LE(12),btl=b.readUInt32LE(20),ft=JSON.parse(b.toString('utf8',28,28+ftl)),bt=JSON.parse(b.toString('utf8',28+ftl,28+ftl+btl));assert.equal(ft.BATCH_LENGTH,rows.length);for(const row of rows){assert.equal(bt.buildingId[row.tileFeatureId],row.buildingId);assert.equal(bt.constructionYear[row.tileFeatureId],row.constructionYear);assert.ok(row.position[0]>24&&row.position[0]<26&&row.position[1]>60&&row.position[1]<61);}const glbOffset=28+ftl+btl;assert.equal(glbOffset%8,0);assert.equal(b.toString('utf8',glbOffset,glbOffset+4),'glTF');total+=rows.length;}
 assert.equal(total,m.length);assert.equal(new Set(m.map(r=>r.buildingId)).size,m.length);const summary=JSON.parse(fs.readFileSync(`${PUBLIC}dataset.json`));assert.equal(summary.joined+summary.unknown,m.length);assert.ok(summary.joined>0);
});
test('Geographic context includes provenance, real labels and neighborhood coverage',()=>{const c=JSON.parse(fs.readFileSync(`${PUBLIC}map/context.json`));assert.ok(c.labels.length>100);assert.ok(c.labels.every(l=>Number.isFinite(l.position[0])&&Number.isFinite(l.position[1])&&l.name));assert.ok(c.neighborhoods.some(n=>n.name==='Kallio'&&n.count>0));assert.ok(c.neighborhoods.length>10);assert.ok(c.source.endpoint.startsWith('https://kartta.hel.fi/'));});
