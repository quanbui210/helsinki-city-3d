...
export function buildingTriangles(building) {
  const polygons=nodes(building,'gml','Polygon');
  const lodOf=polygon=>{let n=polygon;while(n && n!==building){if(/^lod[0-4]/.test(n.localName||'')) return Number(n.localName[3]);n=n.parentNode;}return 0;};
  const lod=Math.max(...polygons.map(lodOf));
  const triangles=[];
  for(const p of polygons.filter(p=>lodOf(p)===lod)) {
