...
<div id="weather-hud" hidden role="status"></div><button id="about" class="text-button">About Lens <span>↗</span></button></header>
...
<aside id="building-card" class="building-card" hidden><button id="close-building" class="close" aria-label="Close building details">×</button><h2 id="building-address"></h2><p id="building-status"></p><button id="building-prompt" class="lens-prompt" hidden>See how loud this street is →</button><button id="cinematic-view" class="lens-prompt" hidden>Cinematic view</button></aside>
<dialog id="about-dialog"><button class="close" id="close-about" aria-label="Close about dialog">×</button><div class="eyebrow">ABOUT HELSINKI LENS</div><h2>A clearer view.<br>Before the viewing.</h2><p>Explore real CityGML buildings from Helsinki and eastern Espoo. Helsinki dates join the building register by RATU; Espoo years come from the CityGML record. No buildings or dates are invented.</p><div class="dialog-stats" id="data-stats"></div><h3>More of the city, honestly represented</h3><p>Central and eastern Helsinki tiles are the <strong>26 March 2019</strong> Kalasatama CityGML crop. Later inner-city strips (Kamppi, Töölö, Ruoholahti, Lauttasaari, Ullanlinna, Pasila and neighbours) come from the official Helsinki city information model WFS. Otaniemi, Keilaniemi and Westend come from the official City of Espoo CityGML LOD2 WFS. Neighborhood shortcuts use Helsinki districts only. Vantaa LOD2 is not openly available, so it is not extruded from 2D footprints. Planned objects are excluded; demolished buildings may remain in the 2019 tiles.</p><p>Ground imagery comes from <strong>National Land Survey orthophotos</strong> when connected; Helsinki’s reference map remains the fallback. Photographic ground and surrounding 2024 mesh remain fixed during the timeline: modern roofs and streets may appear even for earlier selected years.</p><p>This is <strong>not a reconstruction of historical Helsinki</strong>. Buildings demolished before the geometry snapshot are absent. Unknown completion years always remain visible in gray. Source N2000 heights are retained without geoid correction or a surveyed terrain model.</p><h3>Photographic context and illustrative detail</h3><p>Selectable buildings retain their official CityGML geometry. Surrounding photographic mesh is Helsinki’s 2024 model; distant massing is official LOD1. Trees use real street/park register positions with illustrative shapes and heights. Surface materials and lit windows are decorative, not evidence of actual facades, apartments or occupancy. <a href="/foundation.json" target="_blank" rel="noopener">Visual sources and limitations ↗</a></p><h3>Explore with mouse, touch or keyboard</h3><p>Drag to pan. Scroll or pinch to zoom. Shift-drag or right-drag to rotate and tilt. Focus the map and use arrow keys to pan, + / − to zoom, or Home to reset. Press Space to play the timeline. Use the search to find a building, street or neighborhood.</p><h3>Sources</h3><ul><li><a href="https://3d.hel.ninja/data/citygml/" target="_blank" rel="noopener">Helsinki 3D City Model ↗</a></li><li><a href="https://hri.fi/data/fi/dataset/helsingin-rakennukset" target="_blank" rel="noopener">Helsinki building register ↗</a></li><li><a href="https://kartat.espoo.fi/teklaogcweb/wfs.ashx" target="_blank" rel="noopener">Espoo CityGML LOD2 WFS ↗</a></li><li><a href="/dataset.json" target="_blank" rel="noopener">Geometry and join provenance ↗</a></li><li><a href="/map/context.json" target="_blank" rel="noopener">Map labels and geographic provenance ↗</a></li></ul><p class="fine-print">Cities of Helsinki and Espoo · CC BY 4.0. Real CityGML geometry with neutral stone materials. Original facade textures are not included. Data lenses add a soft tint; these are not measured facade appearances.</p></dialog><div id="toast" role="status"></div>`;
const $=s=>document.querySelector(s);
let viewer,tileset,manifest,slider,summary,nav,mapContext,selected,layerManager,layerSwitcher,appearance,selectionRecord,priceAreas,parkingAreas,streetTrees,realityMesh,distantContext;
...
let heroCollapsed=false;try{heroCollapsed=sessionStorage.getItem('lens-hero-collapsed')==='1';}catch{/* private mode */}
if(heroCollapsed){document.body.classList.add('hero-collapsed');$('.intro').hidden=true;}
// Fires once per session on the user's first real map interaction (search, pan/drag,
// zoom or a building click), freeing the hero's vertical space for the neighborhoods panel.
function collapseHero(){
 if(heroCollapsed)return;heroCollapsed=true;try{sessionStorage.setItem('lens-hero-collapsed','1');}catch{/* private mode */}
 document.body.classList.add('hero-collapsed');
 const intro=$('.intro');intro.classList.add('is-collapsed');
 intro.addEventListener('transitionend',()=>{intro.hidden=true;},{once:true});
}
function closeSelection(preserveSearch=false){if(preserveSearch!==true){listingSearch.close();if(mobileDrawerWasOpen!==undefined&&layerSwitcher){layerSwitcher.open=mobileDrawerWasOpen;layerSwitcher.syncOpen();mobileDrawerWasOpen=undefined;}}if(selected){try{selected.color=C.Color.WHITE;}catch{/* A streamed tile may have been evicted. */}selected=undefined;tileset?.makeStyleDirty();}selectionRecord=null;buildingSelection?.clear();cinematic.cancel();$('#building-card').hidden=true;$('#building-prompt').hidden=true;$('#cinematic-view').hidden=true;buildingPanel.closeDetail();}
...
function showBuilding(record,feature){
 closeSelection(true);if(!record){listingSearch.close();return;}selectionRecord=record;collapseHero();nav.orbit=false;
 if(feature){selected=feature;feature.color=C.Color.fromCssColorString('#f8efc7');}
 // The record card's own metric grid already repeats every layer's value for
 // this building, so the full bottom layer drawer is redundant screen space
 // whenever it's open here; the mini-legend stays visible regardless.
 // Recompute --lens-top synchronously right after: getBoundingClientRect
 // forces layout even mid-script, so the building card's height cap reflects
 ...
 // LayerSwitcher already links to; price/parking build their own detail
...
async function init(){
 ...
 const coverage=await get('/tileset/tileset.json');
 streetTrees=new StreetTrees(viewer,await get('/trees.json'),nav);
 loadRealityMesh(viewer,coverage).then(mesh=>{realityMesh=mesh;if(window.__atlas)window.__atlas.realityMesh=mesh;}).catch(()=>toast('Photographic context unavailable; building records remain available.'));
 loadDistantContext(viewer,coverage).then(context=>{distantContext=context;if(window.__atlas)window.__atlas.distantContext=context;}).catch(()=>{});
 ...
 // Tab order follows real shopper priority, not build order: price and noise
 // are what a real apartment-shopper checks first; age/energy/use are
 // supporting context checked after those land; parking last. Overview stays
 // first as the neutral default view.
 layerManager=new LayerManager({tileset,viewer,getRange:()=>nav.state.range,layers:[overviewLayer,createPriceLayer(priceData),createNoiseLayer(layerData),ageLayer,energyLayer,buildingUseLayer,createParkingLayer(parkingData,points=>{const lon=points.reduce((s,p)=>s+p.position[0],0)/points.length,lat=points.reduce((s,p)=>s+p.position[1],0)/points.length;nav.flyTo([lon,lat],Math.max(350,nav.state.range*.5));})].map(presentLayer),manifest});
...
document.addEventListener('keydown',e=>{if(stepInside?.active)return;const editing=['INPUT','SELECT','BUTTON','A'].includes(document.activeElement.tagName);if(e.key==='/'&&!editing){e.preventDefault();$('#map-search').focus();}if(e.code==='Space'&&!editing&&!$('#about-dialog').open&&slider&&layerManager.activeId==='age'){e.preventDefault();slider.playing?slider.pause():slider.start();}
 ...
 if(e.code==='Escape'&&!buildingPanel.detailDialog?.open)closeSelection();});
