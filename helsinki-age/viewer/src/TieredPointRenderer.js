...
export class TieredPointRenderer {
  constructor(viewer,config,getRange){
    ...
    this.entries=config.points.map(p=>({data:p,position:C.Cartesian3.fromDegrees(...p.position,2),primitive:this.points.add({id:{renderer:this,point:p},position:C.Cartesian3.fromDegrees(...p.position,2),pixelSize:7,color:C.Color.fromCssColorString(config.pointColor(p)),outlineColor:C.Color.fromCssColorString('#18343b'),outlineWidth:1})}));this.points.show=false;
    ...
    this.handler=new C.ScreenSpaceEventHandler(viewer.scene.canvas);this.handler.setInputAction(e=>{const picked=viewer.scene.pick(e.position)?.id;this.detail.hidden=true;if(this.active&&picked?.renderer===this&&config.describePoint){this.detail.textContent=config.describePoint(picked.point);this.detail.hidden=false;}},C.ScreenSpaceEventType.LEFT_CLICK);
    this.ready=(async()=>{
  ...
  update(){
    ...
    if(weights.point>0)for(const e of this.entries){e.primitive.color=C.Color.fromCssColorString(this.config.pointColor(e.data)).withAlpha(weights.point);e.primitive.outlineColor=C.Color.fromCssColorString('#18343b').withAlpha(weights.point);}
    ...
    this.clusters=clusterPoints(projected,96);
    while(this.overlay.children.length>this.clusters.length)this.overlay.lastChild.remove();
    this.clusters.forEach((cluster,i)=>{const button=this.overlay.children[i]??document.createElement('button');button.className='tiered-cluster';button.textContent=String(cluster.count);button.style.left=`${cluster.x}px`;button.style.top=`${cluster.y}px`;button.title=`${cluster.count} ${this.config.countLabel} · zoom in`;button.setAttribute('aria-label',button.title);button.onclick=()=>this.config.onCluster(cluster.members.map(m=>m.entry.data));if(!button.parentNode)this.overlay.append(button);});
  }
