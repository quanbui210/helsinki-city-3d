...
`npm run dev` and `npm run preview` include the server endpoint. For production run **`npm run build` then `npm start`** (default port 3000). `server/start.js` serves both the built app and `/api/listings`; an HTTPS reverse proxy should preserve the Host header. A static-only host (CDN, GitHub Pages, Vercel without a function) still serves the map but `POST /api/listings` 404s and the card shows “Search is temporarily unavailable.” On Vercel, keep Root Directory as this app, deploy `api/listings.js`, `api/foundation/config.js`, `api/foundation/aerial.js` and `api/foundation/[...path].js`, and set `BRAVE_SEARCH_API_KEY` / `LISTING_DAILY_LIMIT` / `BRAVE_STORAGE_ALLOWED` / `NLS_API_KEY` on Production (exact names; `LISTING_DAILLY_LIMITIT` is ignored). With the key, the globe uses NLS `ortokuva` via `/api/foundation/aerial?z=&x=&y=` — Vercel does not route `/api/foundation/aerial/{z}/{x}/{y}` to the function. Without the key, `/api/foundation/config` reports `aerial:false` and the globe stays on the old dusk/day PNG. Unconfigured keys, outages and rate limits show manual search links without blocking the record card. The address is sent only to this endpoint and Brave when you click View current listings, never on hover, map load, or record open. Helsinki and Espoo use their own city names; a postal code is included only when supplied by an official address result, not inferred from historical price polygons.
...
Stadiums and Helsinki Cathedral are the first landmark-specific architectural classes. Five curated stadium destinations use dark continuous venue facades, structural ribs, pale roofs and restrained cool concourse lighting instead of apartment windows. Helsinki Cathedral uses cream stone, a patinated green roof and warm dusk illumination. The source CityGML geometry is unchanged; these materials are interpretive and do not claim surveyed facade colors. Landmark identity comes from the prepared OpenStreetMap destination set and a bounded nearest-center match: 45 m for stadiums and 20 m for the Cathedral. `viewer/public/landmark-styles.json` records every match and its distance.

Featured landmark names and Finnish source names are included in the same search box as addresses. A matched result selects the exact modeled building. Apartment-only actions, including listing discovery and Step Inside, are hidden for landmark records. Refresh the classification with `npm run data:landmarks` after `data:nearby`; validate it with `npm run test:landmarks` while the dev server is running.
...
All per-building data stays in the shared `buildings-manifest.json`. Its existing array format is retained for compatibility, with unique `buildingId` keys indexed into a `Map` at runtime. `LayerManager` hydrates properties once for each streamed content object, including tiles reloaded after eviction. A one-byte landmark style attribute is embedded per geometry vertex so the GPU can suppress residential windows for each identified venue; the batch table also carries the human-readable landmark identity. Tests compare actual feature visibility and colors, verify camera continuity and deep links, and exercise desktop/mobile layouts. A local headless Chrome check at 1440×1000 with Noise active, the full manifest and eight streamed tiles measured 34.8 ms median / 39.5 ms p95 frame intervals (about 29 fps). This is not a cross-device frame-rate guarantee. The Cesium production bundle is still about 1.15 MB gzipped; the 7.26 MB uncompressed address index is loaded only when search is focused.
...
## Step Inside: floor view and seasonal sunlight

Select a modeled building and choose **Step inside**. The focused overlay combines floor, facade, look direction and month controls with openness and direct-sun estimates. Drag horizontally or use the look slider to turn; Exit to map or Escape restores the previous camera, lens, lighting and theme. Unmodeled search addresses do not offer this action.

Run `npm run data:floors` to refresh the verified Helsinki WFS field `i_kerrlkm` from `avoindata:Rakennukset_alue_rekisteritiedot`, joined by RATU. Both manifests receive nullable floorCount and floorCountSource; floors-source.json records provenance. Current coverage: 5,669 buildings. Conflicting counts and Espoo records remain unknown; their selector uses an explicitly unconfirmed 1–8 range. The full data:prepare pipeline includes this enrichment.

Geometry analysis decodes the same generated b3dm triangles rendered by Cesium, including off-screen neighboring buildings within 1,200 m. A facade point sits 0.5 m outside an actual wall, at modeled base + (floor − 1) × floor height + 1.5 m. Plausible modeled height/count values of 2–5 m are used; otherwise 3 m is assumed. Negative basement bases use the median of up to eight nearby nonnegative model bases within 200 m, or zero if none exist. This is an explicit estimate, not surveyed terrain. Upper floors of stepped buildings choose a wall segment at the requested height; missing walls produce an unavailable assessment.

Openness uses seven rays across 120°, capped at 120 m: mean distance below 25 m means closely facing another building; below 70 m means partially blocked; otherwise mostly open. Calibration against the current dataset separates Pohjoisesplanadi 21's south park-facing facade (about 91 m mean) from its enclosed north side (about 5 m); Bulevardi 15 and 17 provide additional tightly enclosed examples. These are geometric checks, not verified apartment window views.

SunCalc 1.9 computes solar position at solar noon on the 15th of each selected month in the current year. The local sun vector drives directional lighting and building shadows. Direct sun requires the sun above the horizon, facing the facade, and no modeled obstruction within 1,000 m. Missing tile requests produce an incomplete-data caveat. No daily sunlight-hour totals or interior layouts are inferred.

Limitations: source ages vary; trees are illustrative and are not occlusion inputs; terrain, actual window positions and floor elevations are unverified. The flat ground imagery and source building elevations can visibly diverge at eye level. Openness and sunlight remain estimates even when all local tiles load. A clear ray is not a guarantee of an open view or usable daylight.

Validation: `npm run test:step-inside` (dev server on port 5173) covers real facade placement, floor changes, seasonal sun, restoration, mobile layout and geometry failure. `npm test` includes triangle/solar math and real facade calibration; `npm run build` verifies production bundling. Browser tests mock listing requests.


## Nearby transit and landmarks

Nearby is building context, not another data lens. Building & area starts with the three closest served boarding platforms within 1.5 km, then consolidates same-name, same-mode opposite-direction platforms into one readable option with their combined scheduled lines. Curated landmarks within 800 m have a separate heading. Additional landmarks expand inline; selecting a place moves to it on the map. Raw nearest-three results are retained for every building even when farther than the display radius. Official unmatched address points use the same deterministic calculation inside the prepared coverage. Missing coverage and valid empty results have different messages. Distances are haversine straight-line estimates to a stop or OSM center, not walking routes, entrances, accessibility assessments, or journey times.

Refresh with `npm run data:nearby` (Node 22+ and Python 3, standard library only). HSL's large ZIP is streamed without extracting its files; `stops.txt → stop_times.txt → trips.txt → routes.txt` supplies lines and modes. Station containers, entrances, unserved platforms and pass-through timing points are excluded. Opposite-direction platforms remain separate in the source data and are consolidated only for the building-panel presentation. The snapshot includes scheduled lines across the feed window, not a claim of service today or live departures. Current snapshot: 2,882 served stops and 32 curated landmarks for 16,251 buildings.

The GTFS URL is published directly by [HSL Open Data](https://www.hsl.fi/en/hsl/open-data): https://infopalvelut.storage.hsldev.com/gtfs/hsl.zip. This public static feed needs no key and avoids a runtime GraphQL dependency. Landmarks come from Overpass using the exact-category allowlist in data-pipeline/landmarks.js: major malls, selected churches and stadiums, principal railway stations, Oodi, Ateneum, Kiasma, Sibelius Monument and Suomenlinna. Similar-name cafes, rentals and minor historic markers are excluded. Relation/node duplicates collapse by Wikidata identity or curated name. `OVERPASS_URL` can select another public interpreter endpoint; requests identify the app. Failed/incomplete responses do not produce an empty replacement dataset. Honor endpoint retry delays; no request runs from the user's browser.

`npm run data:nearby:offline` rebuilds from the ignored raw cache. Both manifests retain nearbyTransit and nearbyLandmarks. viewer/public/nearby.json carries the point records, bounds, query, retrieval timestamps, feed dates, provenance and limitations. HSL data is CC BY 4.0; OSM landmark data is ODbL, with source identities and attribution retained in the public file and map credits.

The existing TieredPointRenderer now supports optional pictogram billboards, custom cluster content and no aggregate geometry. Nearby uses larger 140 px screen bins: hidden at city scale, compact mode-pictogram clusters at medium zoom, and individual HSL icons at street scale. Numeric badges are deliberately omitted because they can be mistaken for route or venue numbers; the exact stop density remains available after zooming. Nearby remains active for every lens and suspends during Step Inside/cinematic views. Parking retains its density wash, numeric segment clusters and dot rendering. White/dark-edged mode icons and label backplates remain legible over aerial imagery and colored lenses.

Transit pictograms derive from [HSL's map-style design assets](https://github.com/HSLdevcom/hsl-map-style/tree/master/map-icons) under its CC BY 4.0 design license. Tram is cropped to its pictogram; metro uses an orange adaptation of the train pictogram. Local originals, modifications and source/license metadata are in viewer/public/icons/hsl. The expandable map key and panel identify these as HSL transit. Landmark diamonds remain distinct.

Validation: `npm run test:nearby` (dev server on 5173) checks CSV joins, mode mapping, distance sorting, curated duplicates, all manifest records, three zoom tiers, all lenses, marker selection, nearby empty states, readable daylight labels, the stable Details button position, mobile and Step Inside isolation. Parking regression: `npm run test:parking:tiers`.
