...
`npm run dev` and `npm run preview` include the server endpoint. For production run **`npm run build` then `npm start`** (default port 3000). `server/start.js` serves both the built app and `/api/listings`; an HTTPS reverse proxy should preserve the Host header. Pure static hosting still supports the map but cannot perform listing lookups. Unconfigured keys, outages and rate limits show manual search links without blocking the record card. The address is sent only to this endpoint and Brave when the full card is opened, never on hover or initial map load. Helsinki and Espoo use their own city names; a postal code is included only when supplied by an official address result, not inferred from historical price polygons.
