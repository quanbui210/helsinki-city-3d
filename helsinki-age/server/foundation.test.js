...
import {foundationUrl} from './invoke-foundation.js';
...
test('aerial proxy keeps credentials server-side and deduplicates cached requests',async()=>{
  ...
  const results=await Promise.all([invoke(middleware,'/api/foundation/aerial/12/2331/1185.jpg'),invoke(middleware,'/api/foundation/aerial?z=12&x=2331&y=1185')]);
...
test('query tile URL is reconstructed from Vercel-style req.query',()=>{
  assert.equal(foundationUrl({url:'/api/foundation/aerial',query:{z:'12',x:'2331',y:'1185'}},'/api/foundation/aerial'),'/api/foundation/aerial?z=12&x=2331&y=1185');
});
