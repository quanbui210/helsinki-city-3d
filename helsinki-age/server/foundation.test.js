...
test('aerial proxy keeps credentials server-side and deduplicates cached requests',async()=>{
  ...
  const results=await Promise.all([invoke(middleware,'/api/foundation/aerial/12/2331/1185.jpg'),invoke(middleware,'/api/foundation/aerial/12/2331/1185.jpg')]);
  ...
  assert.equal((await invoke(middleware,'/api/foundation/aerial/19/1/1.jpg')).status,400);
